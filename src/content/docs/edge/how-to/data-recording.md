...
- Connect the field device to the Crawler's network

---
# General workflow

1. Make the field device available in the Crawler's network
...
- The information is displayed later on the device page, but has no further use within the Crawler system.
...
- The "Variable Browser" then displays the available measured variables and their surrounding structure
  ...
  - Measured variables that are configured on the Crawler but were not found on the field device during import are highlighted in ***yellow***
